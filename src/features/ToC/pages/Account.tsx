...
export const Account: React.FC = () => {
        ...
        const run = async () => {
            ...
            if (email) {
                ...
                const { data: user, error: userError } = await supabase
                    .from("user_profiles")
                    .select("id, is_able, user_email, user_type, admin_api_usage_count, paid_api_usage_count, admin_api_usage_updatedAt, paid_api_usage_updatedAt")
                    .eq("user_email", email)
                    .single();
                ...
                if (userError) {
                    ...
                    setFreeCreditDate(
                        format(
                            new Date(new Date(user?.admin_api_usage_updatedAt).setMonth(new Date(user?.admin_api_usage_updatedAt).getMonth() + 1)),
                            'dd.MM.yyyy'
                        )
                    );
                    setPaidCreditDate(format(new Date(new Date(user?.paid_api_usage_updatedAt).setMonth(new Date(user?.paid_api_usage_updatedAt).getMonth() + 1)), 'dd.MM.yyyy'));
                }
                ...
                if (subcriptionError) {
                    console.error(subcriptionError, "subcriptionError");
                    throw new Error(`Failed to subcriptionError: ${subcriptionError.message}`);
                }
                else {
                ...
                console.log(user?.admin_api_usage_count, user?.admin_api_usage_updatedAt, 'ghjgjghjhgjghg65567556')
            }
        }
        run();
        // Set up interval to run the function every 2 seconds
    ...
    return (
        <div className="mt-4" style={{ fontFamily: 'Roboto, sans-serif', backgroundColor: "transparent", padding: "20px", borderRadius: "10px", color: "#fff" }}>
            <Helmet>
            ...
            <section>
                <table style={{ width: "100%", borderCollapse: "collapse", marginTop: "20px" }}>
                    <thead>
                        <tr>
                            <th style={{ textAlign: "left", padding: "12px", fontSize: "1.2em", color: "#4CAF50" }}><h2>Plan</h2></th>
                            <th style={{ textAlign: "left", padding: "12px", fontSize: "1.2em", color: "#4CAF50" }}><h2>Term</h2></th>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td style={{ padding: "12px", textAlign: "center" }}>
                                    ...
                                    (
                                        <>
                                            ...
                                            {userType === 1 &&
                                                <Button
                                                    type="primary"
                                                    onClick={() => { if (isCancelCurrentPlan == null) { navigate("/pricing") } }}
                                                    style={{
                                                        height: "auto",
                                ...
                                {userType !== 1 && (
                                    isCancelCurrentPlan ? ("  Your current plan has been canceled.") : (
                                        <Button
                                            type="primary"
                                            onClick={() => {
                                                if (currentMethod === "stripe") {
            ...
            <div style={{
                ...
                <table style={{ width: "60%", borderCollapse: "collapse", marginTop: "20px", backgroundColor: "#1e1e1e", borderRadius: "10px", boxShadow: "0 2px 5px rgba(0,0,0,0.1)" }}>
                    <tbody>
                        <tr>
            ...
            <Modal
                title={
                    <div style={{ display: 'flex', alignItems: 'center', color: '#f1f1f1' }}>
                        <ExclamationCircleOutlined style={{ color: '#FF7A00', marginRight: '8px', fontSize: '1.5rem' }} />
                ...
                footer={
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                        <Button onClick={handleCancelModal} style={{ background: '#333', color: '#f1f1f1', border: 'none' }}>
                            Cancel
                        </Button>
                        <Button
            ...
            >
                <p style={{ marginBottom: '1rem', textAlign: 'center', fontWeight: '500' }}>
                    To upgrade your plan, kindly cancel your current plan first. Would you like to proceed with this step?
                </p>
            </Modal>
        </div>
    );
